...
$(document).on('click', '.priorityBtn', priorityBtn);
function priorityBtn(e){
	var no = $(this).attr('id').substring(11);
	var value =1-$(this).val();
	var url = "${pageContext.request.contextPath}/board/priority"+'?'+$.param({"no": no, "priority" : value});
	$.ajax({
		type :'PUT',
		url : url, 
			success : function(data){
		 	 ...
		 	 $(".star"+no).remove();
	 	 	 if(value==1){
	 	 		 $("#priorityBtn"+no).val(1);
				 $("#priorityBtn"+no).append("<span class='\glyphicon glyphicon-star star"+no+" '\></span>");
			 }else{
				 $("#priorityBtn"+no).val(0);
				 $("#priorityBtn"+no).append("<span class='\glyphicon glyphicon-star-empty star"+no+" '\></span>"); 
			 }  
 		},
		error : function(data){
			console.log("수정 실패");
...
for (var i = 0; i < boardList.length; i++) {
	...
	if($("."+boardList[i].duedate).length <=0){
	    ...
	    var $list =$("<div style='\margin-left: 46%; margin-right:46%; margin-bottom: 0.5rem; margin-top: 0.5rem;'\>"+boardList[i].duedate+"</div>"
	    	      ...
	    	      +"<button class='\ btn btn-default btn-xs priorityBtn'\ id='\priorityBtn"+boardList[i].no+"'\  value='\ "+boardList[i].priority+" '\ "
	    	      +"style='\margin-bottom: 0.3rem;'\> </button>"
	...
	if(boardList[i].priority==0){
		$("#priorityBtn"+boardList[i].no).append("<span class='\glyphicon glyphicon-star-empty star"+boardList[i].no+" '\></span>");
	}else{
		$("#priorityBtn"+boardList[i].no).append("<span class='\glyphicon glyphicon-star star"+boardList[i].no+" '\></span>");
	}
}
$(".badge").text(cnt);
